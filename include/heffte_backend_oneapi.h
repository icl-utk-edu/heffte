...
template<> struct one_dim_backend<backend::onemkl>{
    ...
    static std::unique_ptr<mkl_executor> make(sycl::queue *, box3d<index> const box, int dimension){
        return std::unique_ptr<mkl_executor>(new mkl_executor(box, dimension));
    }
    //! \brief Constructs a real-to-complex executor.
    template<typename index>
    static std::unique_ptr<mkl_executor_r2c> make_r2c(sycl::queue *, box3d<index> const box, int dimension){
