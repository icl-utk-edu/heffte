...
template<typename scalar_type> struct plan_cufft{
     ...
     * \param dist is the distance between the first entries of consecutive sequences
     */
    plan_cufft(cudaStream_t stream, int size1, int size2, int batch, int stride, int dist){
        size_t work_size = 0;
    ...
    void make_plan(std::unique_ptr<plan_cufft<scalar_type>> &plan) const{
        if (not plan){
            if (size2 == 0)
                ...
                plan = std::unique_ptr<plan_cufft<scalar_type>>(new plan_cufft<scalar_type>(stream, size, size2, howmanyffts, stride, dist));
        }
    }
...
template<> struct one_dim_backend<backend::cufft>{
    ...
    template<typename index>
    static bool can_merge(box3d<index> const &box, int dir1, int dir2){
        int corder1 = box.find_order(dir1);
        int corder2 = box.find_order(dir2);
        return std::min(corder1, corder2) != 0 or std::max(corder1, corder2) != 2;
    }
