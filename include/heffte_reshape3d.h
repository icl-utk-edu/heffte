...
class reshape3d_alltoallv : public reshape3d_base, backend::auxiliary_variables<backend_tag>{
public:
    ...
    make_reshape3d_alltoallv(typename backend::auxiliary_variables<b>::queue_type, std::vector<box3d<i>> const&, std::vector<box3d<i>> const&, MPI_Comm const);
...
private:
     ...
     */
    reshape3d_alltoallv(typename backend::auxiliary_variables<backend_tag>::queue_type q,
 ...
 */
template<typename backend_tag, template<typename device> class packer, typename index>
class reshape3d_pointtopoint : public reshape3d_base, backend::auxiliary_variables<backend_tag>{
public:
    //! \brief Destructor, frees the comm generated by the constructor.
    ...
    make_reshape3d_pointtopoint(typename backend::auxiliary_variables<b>::queue_type, std::vector<box3d<i>> const&, std::vector<box3d<i>> const&, MPI_Comm const);
...
private:
     ...
     * \brief Private constructor that accepts a set of arrays that have been pre-computed by the factory.
     */
    reshape3d_pointtopoint(typename backend::auxiliary_variables<backend_tag>::queue_type stream,
...
public:
    ...
    reshape3d_transpose(typename backend::auxiliary_variables<typename backend::from_location<location_tag>::type>::queue_type q,
        ...
        backend::auxiliary_variables<typename backend::from_location<location_tag>::type>(q),
        plan(cplan)
        {}
...
private:
    ...
    void transpose(scalar_type const *source, scalar_type *destination, scalar_type *workspace) const{
        if (source == destination){ // in-place transpose will need workspace
            ...
            transpose_packer<location_tag>().unpack(this->gpu_queue(), plan, workspace, destination);
        }else{
            transpose_packer<location_tag>().unpack(this->gpu_queue(), plan, source, destination);
        }
    }
...
std::unique_ptr<reshape3d_base> make_reshape3d(typename backend::auxiliary_variables<backend_tag>::queue_type stream,
                                               std::vector<box3d<index>> const &input_boxes,
                                               std::vector<box3d<index>> const &output_boxes,
                                               MPI_Comm const comm,
    ...
    if (match(input_boxes, output_boxes)){
        if (input_boxes[0].ordered_same_as(output_boxes[0])){
            ...
            return std::unique_ptr<reshape3d_base>(new reshape3d_transpose<typename backend::buffer_traits<backend_tag>::location, index >(stream, plans[0]));
        }
    }else{
