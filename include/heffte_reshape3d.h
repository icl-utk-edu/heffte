...
class reshape3d_alltoallv : public reshape3d_base, public backend::device_instance<backend_tag>{
public:
 ...
 */
template<typename backend_tag, template<typename device> class packer, typename index>
class reshape3d_pointtopoint : public reshape3d_base, public backend::device_instance<backend_tag>{
public:
    //! \brief Destructor, frees the comm generated by the constructor.
...
class reshape3d_transpose : public reshape3d_base, public backend::device_instance<typename backend::default_backend<location_tag>::type>{
public:
...
private:
    ...
    void transpose(scalar_type const *source, scalar_type *destination, scalar_type *workspace) const{
        if (source == destination){ // in-place transpose will need workspace
            backend::data_manipulator<location_tag>::copy_n(this->stream(), source, size_intput(), workspace);
...
std::unique_ptr<reshape3d_base> make_reshape3d(typename backend::device_instance<backend_tag>::stream_type stream,
                                               std::vector<box3d<index>> const &input_boxes,
                                               std::vector<box3d<index>> const &output_boxes,
                                               MPI_Comm const comm,
