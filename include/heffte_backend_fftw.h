...
template<> struct one_dim_backend<backend::fftw>{
    ...
    template<typename index>
    static std::unique_ptr<fftw_executor> make(void*, box3d<index> const&, int, int){
        throw std::runtime_error("2d fftw executor not implemented");
        return std::unique_ptr<fftw_executor>();
    }
