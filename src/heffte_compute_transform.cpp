    ...
    auto apply_fft = [&](int i, scalar_type data[])
        ->void{
            ...
            if (dir == direction::forward){
                if (executor[i] != nullptr) executor[i]->forward(data, executor_workspace);
            }else{
                if (executor[i] != nullptr) executor[i]->backward(data, executor_workspace);
            }
        };
...
void compute_transform(typename backend::data_manipulator<location_tag>::stream_type,
    ...
    if (last < 1){ // no reshapes after 0
        ...
        if (executor[0] != nullptr) executor[0]->forward(effective_input, output, executor_workspace);
        if (executor[1] != nullptr) executor[1]->forward(output, executor_workspace);
        if (executor[2] != nullptr) executor[2]->forward(output, executor_workspace);
        return;
    }
    ...
    { add_trace name("fft-1d");
    if (executor[0] != nullptr) executor[0]->forward(effective_input, temp_buffer, executor_workspace);
    }
    ...
    for(int i=last; i<3; i++){
        add_trace name("fft-1d");
        if (executor[i] != nullptr) executor[i]->forward(output, executor_workspace);
    }
}
template<typename location_tag, typename index, typename scalar_type>
void compute_transform(typename backend::data_manipulator<location_tag>::stream_type stream,
    ...
    if (shaper[0]){
        ...
        add_trace name("copy");
        int valid_executor = (executor[0] != nullptr) ? 0 : ((executor[1] != nullptr) ? 1 : 2);
        backend::data_manipulator<location_tag>::copy_n(stream, input, executor[valid_executor]->box_size(), temp_buffer);
    }

    for(int i=0; i<2; i++){ // apply the two complex-to-complex ffts
        { add_trace name("fft-1d x3");
        if (executor[i] != nullptr) executor[i]->backward(temp_buffer, executor_workspace);
        }
        if (shaper[i+1]){
            add_trace name("reshape");
...
#define heffte_instantiate_transform(location_tag, index) \
    template void compute_transform<location_tag, index, std::complex<float>>( \
                        typename backend::data_manipulator<location_tag>::stream_type, \
