...
template<typename backend_tag, typename index>
fft3d_r2c<backend_tag, index>::fft3d_r2c(typename backend::auxiliary_variables<backend_tag>::queue_type gpu_stream,
                                         logic_plan3d<index> const &plan, int const this_mpi_rank, MPI_Comm const comm) :
    backend::auxiliary_variables<backend_tag>(gpu_stream),
    pinbox(new box3d<index>(plan.in_shape[0][this_mpi_rank])), poutbox(new box3d<index>(plan.out_shape[3][this_mpi_rank])),
    scale_factor(1.0 / static_cast<double>(plan.index_count))
    #ifdef Heffte_ENABLE_MAGMA
    , hmagma(this->gpu_queue())
...
template<typename backend_tag, typename index>
void fft3d_r2c<backend_tag, index>::setup(logic_plan3d<index> const &plan, MPI_Comm const comm){
    for(int i=0; i<4; i++){
    ...
    executor_r2c = one_dim_backend<backend_tag>::make_r2c(this->gpu_queue(), plan.out_shape[0][mpi::comm_rank(comm)], plan.fft_direction[0]);
    executor[0] = one_dim_backend<backend_tag>::make(this->gpu_queue(), plan.out_shape[1][mpi::comm_rank(comm)], plan.fft_direction[1]);
    executor[1] = one_dim_backend<backend_tag>::make(this->gpu_queue(), plan.out_shape[2][mpi::comm_rank(comm)], plan.fft_direction[2]);
}
...
heffte_instantiate_fft3d_r2c(backend::onemkl, int)
heffte_instantiate_fft3d_r2c(backend::onemkl, long long)
#endif
