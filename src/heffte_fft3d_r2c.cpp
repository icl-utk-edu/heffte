...
template<typename backend_tag, typename index>
fft3d_r2c<backend_tag, index>::fft3d_r2c(typename backend::device_instance<backend_tag>::stream_type gpu_stream,
...
void fft3d_r2c<backend_tag, index>::standard_transform(std::complex<scalar_type> const input[], scalar_type output[],
    ...
    if (backward_shaper[0]){
        ...
        add_trace name("copy");
        backend::data_manipulator<location_tag>::copy_n(this->stream(), input, executor[1]->box_size(), temp_buffer);
    }
