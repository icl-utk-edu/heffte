...
/*!
 * \brief HeFFTe example 1, simple DFT using two MPI ranks and stock backend.
 *
...
void compute_dft(MPI_Comm comm){
    ...
    if (num_ranks != 2){
        if (me == 0) std::cout << " heffte_example_stock is set to 2 mpi ranks, exiting \n";
        return;
    }
    ...
    heffte::fft3d<heffte::backend::stock> fft(my_box, my_box, comm);
    ...
    heffte::fft3d<heffte::backend::stock>(world_box, world_box, single_rank_comm)
