/*
    -- heFFTe (version 0.2) --
       ...
       @date
       Testing inplace C2C Fast Fourier Transform on distributed CPUs
       @author Alan Ayala
*/
...
int main(int argc, char *argv[]) {
  ...
  heffte_allocate(HEFFTE_MEM_CPU, &work,  opts.workspace[0], opts.nbytes);     // input/output 3D array
  ...
  if (opts.mode == 0) {
    for (int i = 0; i < opts.nloop; i++) {
      heffte_execute(fft, work, work, FORWARD);   // Forward C2C FFT computation
      heffte_execute(fft, work, work, BACKWARD);  // Backward C2C FFT computation
    }
  } else if (opts.mode == 1) {
      for (int i = 0; i < opts.nloop; i++)
        heffte_execute(fft, work, work, FORWARD);  // Forward C2C FFT computation
