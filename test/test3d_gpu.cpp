 ...
 * Testing HEFFTE library on GPUs
...
// MPI initialization
  ...
  heffte_allocate(1, &dwork, fftsize, nbytes);
  ...
  cudaMemcpy(dwork, work, nbytes, cudaMemcpyHostToDevice);
...
// Copy output back to the CPU
  ...
  cudaMemcpy(work, dwork, nbytes, cudaMemcpyDeviceToHost);
...
if (me == 0){
  ...
  if (me == 0){
    ...
    printf("---------------- \n");
    printf("\tNumber of GPU devices per node: %d\n", ndevices);
